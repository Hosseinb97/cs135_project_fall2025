...
To evaluate and select the best classifier configuration, we used a {{K}}-fold StratifiedGroupKFold cross-validation procedure with author-level grouping.
...
We performed hyperparameter tuning via GridSearchCV, which exhaustively searched across the specified parameter grid and automatically refit the best model on the full training data.
All models were trained exclusively on the provided x\_train.csv and y\_train.csv data; test data were used only for final leaderboard prediction.

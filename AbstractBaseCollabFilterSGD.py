...
class AbstractBaseCollabFilterSGD(object):
    ...
    def evaluate_perf_metrics(self, user_id_N, item_id_N, ratings_N):
        ...
        rmse = np.sqrt(np.mean(np.square(yhat_N - ratings_N)))
    ...
    def fit(self, train_data_tuple, valid_data_tuple=None, do_warm_start=False):
        ...
        if do_warm_start:
            epoch_start = int(np.ceil(self.trace_epoch[-1]))
        else:
            epoch_start = 0
            self.trace_epoch = []
            ...
            self.trace_rmse_train = []
            self.trace_rmse_valid = []

            self.all_loss = []

            ## Store list of L1 norms for each parameter
            ...
            ## Store list of L1 norms of GRADIENT of each parameter
            self.trace_grad_norm_per_param = dict()
            for key in self.param_dict.keys():
                self.trace_grad_norm_per_param[key] = list()
            self.trace_smooth_grad_norm_per_param = dict()
            for key in self.param_dict.keys():
                self.trace_smooth_grad_norm_per_param[key] = list()

        for epoch_count in range(epoch_start, epoch_start + self.n_epochs): 
            ...
            for i, batch_tuple in enumerate(batch_loader):
                ...
                do_report_now = self.check_if_report_progress_now(
                    epoch_count, epoch_start + self.n_epochs, i, batch_loader.n_batches)
                if do_report_now:
                    self.trace_epoch.append(epoch)
                    self.trace_loss.append(loss)
                    self.trace_step_size.append(self.step_size)
                    ...
                    self.trace_rmse_train.append(train_perf_dict['rmse'])
                    self.trace_rmse_valid.append(valid_perf_dict['rmse'])
                    ...
                    # Compute L1 norm of each parameter
                    ...
                    for key, arr in grad_dict.items():
                        norm = np.mean(np.abs(arr))
                        self.trace_grad_norm_per_param[key].append(norm)
